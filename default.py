# -*- coding: utf-8 -*-
import xbmc, xbmcgui, re, xbmcplugin, json, random
...
videodata_url = 'http://amfphp.indavideo.hu/SYm0json.php/player.playerHandler.getVideoData/'

def get_login(params):
    try:
        if (user == '' or password == ''): raise Exception()    
        post = urlencode({'partner_id': 'indavideo', 'redirect_to': base_Url, 'username': user, 'password': password})
...
def build_search_result():
    ...
    post = urlencode({'p_uni': page, 'action': 'search', 'search': search_text, 'view': 'detailed', 'sort_mode': 'magic'})
    ...
    for item in episodes:
        try:
            ...
            addFile(py2_encode(title), link, 7, img, fanart, py2_encode(plot), duration, '1')
        except:
            pass
...
def browse():
    post = urlencode({sort_by[msort]: page})
...
def getMyLists():
    ...
    for item in episodes:
        try:
            title, link, img, plot, duration = getEpisode(item)
            addFile(py2_encode(title), link, 7, img, fanart, py2_encode(plot), duration, act)
        except:
            pass
    control.content(int(sys.argv[1]), 'movies')
    try:
...
def watchLater(url, action, silent = ''):
    try:
        title = url.split('/')[-1]
        post = urlencode({'op': 'watchlater', 'action': 'videoHandler', 'addOrRemove': action, 'url_title': title})
...
def subscribe():
    try:
        ...
        post = urlencode({'op': 'subscribe', 'addOrRemove': action, 'digest_frequency': '10', 'uid': 'null', 'CSRF': ''})
...
def getvideo(url, action, name, iconimage):
    ...
    emb_hash = py2_encode(emb_hash)
    rnd_number = random.randrange(2147483647)
    query = "%s%s/?_=%d" % (videodata_url, emb_hash, rnd_number)
    ...
    for video_url in video_urls:
        try: video_url = py2_encode(video_url)
        except: pass
        height = _search_regex(r'\.(\d{3,4})\.mp4(?:\?|$)', video_url)
        if filesh:
            if not height:
                continue
            ...
            q = '&token={}'
            #q = '?&channel=main-k&zone=hu' + q if ".mp4" in video_url else q
...
def addDir(name, url, mode, iconimage, fanart, description, page, action):
    u=sys.argv[0]+"?url="+quote_plus(url)+"&mode="+str(mode)+"&name="+quote_plus(name)+"&iconimage="+quote_plus(iconimage)+"&fanart="+quote_plus(fanart)+"&description="+quote_plus(description)+"&page="+quote_plus(page)+"&action="+quote_plus(action)
    ok=True
...
def addFile(name, url, mode, iconimage, fanart, description, duration, action):
    ...
    u=sys.argv[0]+"?url="+quote_plus(url)+"&mode="+str(mode)+"&name="+quote_plus(name)+"&iconimage="+quote_plus(iconimage)+"&fanart="+quote_plus(fanart)+"&description="+quote_plus(description)+"&duration="+str(duration)+"&action="+quote_plus(action)
    ok=True
    liz=xbmcgui.ListItem(name)
    liz.setArt({'icon': iconimage, 'thumb': iconimage})
    ...
    if (action == '1' and login == '1'):
        cm.append((u'Hozz\u00E1ad\u00E1s a megn\u00E9zend\u0151kh\u00F6z', 'RunPlugin(%s?mode=5&url=%s&action=%s&name=%s)' % (sysaddon, url, '1', py2_decode(name))))
        cm.append((u'Elt\u00E1vol\u00EDt\u00E1s a megn\u00E9zend\u0151kb\u0151l', 'RunPlugin(%s?mode=5&url=%s&action=%s&name=%s)' % (sysaddon, url, '0', py2_decode(name))))
    ...
    if not action == '0':
        ...
        if not control.setting('savefolder') == 'Nincs megadva!':
            cm.append((u'Let\u00F6lt\u00E9s', 'RunPlugin(%s?mode=7&url=%s&action=%s&name=%s&iconimage=%s)' % (sysaddon, url, '5', py2_decode(name), iconimage)))
    liz.addContextMenuItems(cm, replaceItems=True)
